...
def rnn_step_backward(dnext_h, cache):
    ...
    #Pull cache values for backward pass
    x, prev_h, Wx, Wh, b, next_h = cache
    
    #gradient of loss
    dnext_raw = (1-next_h**2) * dnext_h
    
    #gradient for bias vector
    db = np.sum(dnext_raw)
    
    #grad for dWh
    dWh = np.dot(prev_h.T, dnext_raw)
    
    #grad for dWx
    dWx = np.dot(x.T, dnext_raw)
    
    #grad for previous hidden states
    dprev_h = np.dot(dnext_raw, Wh.T)
    
    #grad for dx
    dx = np.dot(dnext_raw, Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    #get dimensions and store in variables T, D, N, and H
    N, T, D = x.shape
    H = Wh.shape[0]
    h = np.zeros((N,T,H))
    forward_cache = {}
    
    #For each input vector call the rnn_step_forward function on the current vector 
    for t in range(T):
...
def rnn_backward(dh, cache):
    ...
    dprev_h = 0
    #for each input row call the rnn_step_backward function on an augmented dh that includes the previous dh
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #find derivative dnext_c and dprev_c
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]):
        # Compute hidden and state at timestep t
        # append cache at current timestep to list
        h, c, cache_t = lstm_step_forward(x[:, t], hs[-1], c, Wx, Wh, b)
        hs.append(h)
        cache.append(cache_t)
    
    # Stack along T, excluding initial state
